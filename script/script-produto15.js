...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 3. Configuração dos botões de checkout - USANDO SLUG
    // Botão Comprar Agora
    const buyNowButton = document.querySelector('.buy-now-button');
    if (buyNowButton) {
        buyNowButton.addEventListener('click', () => {
            showNotification(`Iniciando a Compra Rápida para "${product.name}"...`);
            // SALVA O PRODUTO NO LOCALSTORAGE E REDIRECIONA
    ...
    // 4. Lógica para o ícone do Carrinho (Modal Centralizado)
    ...
    // 5. LÓGICA DO MODO ESCURO (Mantém o localStorage)
